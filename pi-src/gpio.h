...
// Struct to make it easy to access memory involving GPIOs
// This was devised from pages 90 to 91 in BCM2835-ARM-Peripherals.annot.PDF
struct gpios{
    unsigned int fsel[6];
    ...
    unsigned int eds[2]; // event detect status
    unsigned int reservedE;
    unsigned int ren[2]; // rising edge detect enable
    unsigned int reservedF;
    unsigned int fen[2]; // falling edge detect enable
    unsigned int reservedG;
    unsigned int hen[2]; // high detect enable
    unsigned int reservedH;
    unsigned int len[2]; // low detect enable
    unsigned int reservedI;
    unsigned int aren[2]; // async rising edge detect
    unsigned int reservedJ;
    unsigned int afen[2]; // async falling edge detect
    unsigned int reservedK;
    unsigned int pull[3]; // pull-up/pull-down enable

};

// GPIO memory starts at 0x20200000 for the Raspberry Pis
volatile static struct gpios *gpio = (struct gpios *)0x20200000;
